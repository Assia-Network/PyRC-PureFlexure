...
def generar_reporte_viga( tupla_date, diseno, autor, f_c, f_y, Es, b, h, ecu, diametro_long, diametro_est, r_libre, coef_pbal, Mdise, MU, peralte, ps, p_min, p_min_final, beta1, pb, coef_pbal_real, pmax, p_dise):
    ...
    return rf"""
        ...
        Se realiza la verificación normativa de cuantías para definir el refuerzo. Primero, se controla la \textbf{{cuantía requerida}} frente a la \textbf{{cuantía mínima}}, adoptando el mayor valor para garantizar la resistencia mínima de la sección:
        ...
        Se establece la \textbf{{cuantía de diseño}} preliminar: \textbf{{{p_dise:.4f}}}.

        A continuación, se valida la ductilidad comparando con la \textbf{{cuantía máxima}}. Si $\rho_{{diseño}} \le \rho_{{max}}$, se confirma el diseño como sección \textbf{{simplemente armada}}; si la cuantía requerida supera a la máxima, la sección requeriría acero a compresión (\textbf{{doblemente armada}}).
        ...
        Verificada la condición de sección simplemente armada, se determina el área de refuerzo a tracción:

        \[
        A_{{acero}} = \rho_{{diseño}} \cdot b \cdot d
        \]

        Operando los valores, se obtiene un área de acero de \textbf{{{p_dise*b*peralte:.2f}}} $\text{{cm}}^2$.
        ...
        A continuación, se calcula el área de acero real propuesta ($A_{{s,prop}}$) y se verifica si satisface la demanda:

        \[
        A_{{s,prop}} = N \cdot \frac{{\pi \cdot \phi^2}}{{4}} = {{{varillas_totales}}} \cdot \frac{{\pi \cdot {{{diametro_long*2.54:.2f}}}^2}}{{4}}
        \]
