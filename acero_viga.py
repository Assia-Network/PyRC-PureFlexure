...
if len(n_por_capa_arr) == capas and 1 not in n_por_capa_arr:
    ...
    if all(ancho_consumible >= x for x in (n_por_capa_arr * diame_long_cm + (n_por_capa_arr - 1) * espacimiento_var)) and alto_consumible >= alto_acero:
        ...
        if all(ancho_consumible < x for x in (n_por_capa_arr * diame_long_cm + (n_por_capa_arr - 1) * espacimiento_var)):
            st.markdown("""
...
try:
    ...
    if pmax<p_dise:
        raise ValueError("Se debe diseñar doblemente armado, forzando el except")  #Siguiente version
